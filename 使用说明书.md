...
0. **一键环境准备（可选）**
   ...
   > ⚠️ **Windows 提示**：在 PowerShell/CMD 中默认没有 `bash` 命令。请在 **Git Bash**（Windows 版 Git 自带）
   > 或 **Windows Subsystem for Linux (WSL)** 中执行上述脚本。如果仍希望在 PowerShell 中操作，可使用
   > `wsl bash scripts/bootstrap.sh ...`，或直接运行 `& .\.venv\Scripts\python.exe -m pip install --upgrade -r
   > python/requirements.txt` 来升级虚拟环境内的依赖。
